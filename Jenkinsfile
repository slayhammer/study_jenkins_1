...
//		stage('Build and deploy the app') {
			agent {
				docker {
					...
					args '--group-add docker -v /var/run/docker.sock:/var/run/docker.sock -e JENKINSUID=$(id -u jenkins) -e JENKINSGID=$(id -g jenkins) -e DOCKERGID=$(stat -c %g /var/run/docker.sock)'
				}
			}
